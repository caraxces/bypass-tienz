...
const AuditResults: React.FC<AuditResultsProps> = ({ results, totalCrawled, filterTitle }) => {
  const resultCount = results.length;

  return (
    <div className="flow-root">
       <div className="mb-4">
        <h2 className="text-xl font-semibold">{filterTitle}</h2>
        <p className="text-sm text-gray-600">
          Hiển thị {resultCount} kết quả trong tổng số {totalCrawled} trang đã được crawl.
        </p>
      </div>
      {results.length === 0 ? (
        <p>Không có dữ liệu để hiển thị cho bộ lọc này.</p>
      ) : (
